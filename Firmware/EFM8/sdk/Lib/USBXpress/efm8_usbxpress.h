 ...
 * ### Memory Usage
 *
 *  The USBXpress library consumes approximately 7K of flash, 100 bytes of
 *  XDATA and 65 bytes of IRAM.
 *
 * ## Theory of operation:
 *
 ...
 *
 *   This driver wraps the EFM8 USB Library and both can not be used at the same time.
 *
 ...
 *
 *   For example if the main loop calls into the library and that call is
 *   interrupted by code which also calls into the library an error may occur.
 *   The user must ensure that the library is accessed from only one context at
 *   a time.
 *
 *   The most common way of handling this is to disable USB interrupts when
 *   calling into the library from the main loop. If other ISR's call into the
...
#ifndef  EFM8_USBXPRESS_H
#define  EFM8_USBXPRESS_H
 ...
 *
 ******************************************************************************/
#define USBX_STATUS_OK                   0      //!< Success
#define USBX_STATUS_EP_BUSY             -2      //!< Failed because the endpoint is busy
#define USBX_STATUS_ILLEGAL             -4      //!< Failed due to an illegal parameter
#define USBX_STATUS_EP_STALLED          -5      //!< Failed because the endpoint is stalled
#define USBX_STATUS_DEVICE_UNCONFIGURED -9      //!< Failed because the device is not configured
...
/***************************************************************************//**
 * Constants for use in string descriptor definitions
 ******************************************************************************/

/// Standard USB String Descriptor
#define USBX_STRING_DESCRIPTOR_UTF16LE          0

/// Packed USB String Descriptor
#define USBX_STRING_DESCRIPTOR_UTF16LE_PACKED   1

/// USB String Descriptor Type
#define USBX_STRING_DESCRIPTOR                  3

/***************************************************************************//**
 *  @brief Macro for creating USB-Compliant UTF-16LE UNICODE string descriptor.
 *
 *  @n Example:
 *     USBX_STRING_DESC(iManufacturer[], "Silicon Labs");
 *  @note This macro should be used for UTF-8 strings in which all characters
 *        are represented by a single ASCII byte (i.e. U.S. English strings).
 *
 *        If characters in the string require two bytes to encode, the string
 *        must be defined manually as a uint8_t array. Below is an example of
 *        such a definition, where the trademark symbol is included in the
 *        string:
 *
 *        #define MFR_STRING_LEN sizeof ("Silicon Labs™") * 2
 *
 *        // Manufacturer String
 *        const uint8_t code USB_MfrStr[] =
 *        {
 *          USBX_STRING_DESCRIPTOR_UTF16LE,
 *          MFR_STRING_LEN,
 *          USBX_STRING_DESCRIPTOR,
 *          'S', 0,
 *          'i', 0,
 *          'l', 0,
 *          'i', 0,
 ...
 *          'b', 0,
 *          's', 0,
 *          0x22, 0x21,     // The Trademark symbol is 0x2122 in UTF16
 *        };
 *
******************************************************************************/
#define USBX_STRING_DESC(__name, __val) \
  SI_SEGMENT_VARIABLE(__name,  const uint8_t, SI_SEG_CODE) =  \
    { USBX_STRING_DESCRIPTOR_UTF16LE_PACKED,                  \
      sizeof(__val) * 2,                                      \
      USBX_STRING_DESCRIPTOR,                                 \
      __val }
 ...
 *
 *****************************************************************************/
void USBX_init(USBX_Init_t* p) large;
 ...
 * been sent.
 *
 * This function utilizes the EFM8 USB Library's USBD_Write() function, which
 * only prepares for a write.  The actual write will occur over time as the
 * host transmits data.
 *
 *****************************************************************************/
int8_t USBX_blockWrite(uint8_t* block,
                       uint16_t numBytes,
                       uint16_t* countPtr) large;
 ...
 *
 * This function utilizes the EFM8 USB Library's USBD_Read() function,
 * which only prepares for a read.  The actual read will occur over time as the
 * host transmits data.
 *
 *****************************************************************************/
int8_t USBX_blockRead(uint8_t* block,
                      uint16_t numBytes,
                      uint16_t* countPtr) large;
 ...
 *
 *****************************************************************************/
uint32_t USBX_getCallbackSource(void) large;
 ...
 *
 *****************************************************************************/
void USBX_apiCallbackEnable(USBX_apiCallback_t f) large;
 ...
 *
 *****************************************************************************/
void USBX_apiCallbackDisable(void) large;
 ...
 *
 *****************************************************************************/
uint16_t USBX_getLibraryVersion(void) large;
